...
const is_array = function (value: any) {
    return (
        value &&
        typeof value === 'object' &&
...
// @ts-ignore
const arrayKeyValueToHash = (all: any) =>
    all.reduce(
        ([cr, key]: any, val: any) => (key && [{
            ...cr,
            [key]: val
        }]) || [cr, val],
        [{}],
...
const cb2promise = (resolve: any, reject: any, hook: (arg0: any) => void) => (err: any, res: any) => {
    if (!err) {
        if (hook) {
...
const removeDeletes = (o: { [x: string]: any; }) => {
    Object.keys(o)
        .forEach(k => {
...
const str2bool = (o: string | boolean) => {
    if (o === 'true') {
        o = true;
    } else if (o === 'false') {
        o = false;
...
const unflattenExt = (o: any, types: { [x: string]: any; }) => {
    if (!o) {
        return o;
    }
...
const getRedlock = (redisClientInstance: any) => {
    if (!redLockInstance) {
...
export class RankingField {
    private type: string;
    private rankingField: any;
    private filteringKeys: any;
...
export class RedisStore {
    private logger: any | undefined = undefined;
    ...
    private pathIndexes: string;
    private pathMetadata: string;
    private defaultRankingIndex: {};
    private rankingField: {
        rankingField: any;
        filteringKeys: any | undefined;
    } | undefined;
    private types: any;
    private _lock: any;
    ...
    constructor(parent: any, type: any = undefined,
        ...
        try {
            ...
            if (global.redisClientMainInstance === null) {
                let redisClientFactory: any = null;
                if (parent.config) {
    ...
    throw_error(msg: string | undefined, reject: ((arg0: any) => void) | undefined = undefined) {
    ...
    doc(docName: string | undefined = undefined): any {
        try {
            ...
            if (this.id && this.docType === TYPE_COLLECTION) {
                const pathNames: any = docName && docName.split('/');
    ...
    collection(collectionName: any, type = TYPE_COLLECTION): any {
    ...
    SortedCollection(collectionName: any, rankingField: any) {
        // this.logger.log(' collection','on proxy ', this.pathName,'type',this.docType);
        if (this.id && this.docType === TYPE_DOCUMENT) {
    ...
    getRankingFieldName(rankingField: any, filteringKeys: any) {
    ...
    withRankingField(rankingField: any, filteringKeys: any | undefined = undefined) {
    ...
    persist(path: string | undefined = undefined) {
        if (!path) {
            path = this.pathName;
        }
    ...
    expire(path: string, time:number) {
        if (!path) {
            path = this.pathName;
        }
        this.getRedisClient()
    ...
    expireIn(timeout:number) {
        this.timeout = timeout;
        this.getRedisClient()
            .expire(this.pathName, this.timeout);
    }

    formatIndexSubKeys(index: any, data: any) {
        if (index.filteringKeys && index.filteringKeys.map) {
            const values = index.filteringKeys.map((k: string) => {
                let val;
                if (data && !!data[k]) {
                    val = data[k];
                } else if (data) {
    ...
    formatIndexKey(index: { type: any; rankingField: any; },
    ...
    getRankingPoolKey(index: any, indexMap: any, path: string | undefined = undefined) {
        if (!path) {
    ...
    getRankingPoolId(index: { type: any; rankingField: any; }, path: string | undefined = undefined) {
        if (!path) {
            path = this.parent.pathName;
        }
    ...
    async updateIndexValue(indexMap: any, merge: boolean) {
        try {
            const indexes: any = await this.getDocumentIndexes(this.parent.pathIndexes);
            let fullData: any = null;
            if (indexes && indexes.map) {
                const prs = indexes.map(async (index: { type: any; rankingField: any; filteringKeys?: any; }) => {
                    switch (index.type) {
                        case IDX_TYPE_RANK:
    ...
    async getRankingCount0(data: any) {
        const me = this;
        let err = '';
        try {
            // @ts-ignore
    ...
    async getRankingCount(data: any, min = '-inf', max = '+inf') {
        const me = this;
    ...
    rankingRange(min: any, max: any, data: any) {
        const me = this;
        const index = me.defaultRankingIndex;
        const key = me.getRankingPoolKey(index, data, this.pathName);
        ...
        return new Promise((resolve: (arg0: any) => void, reject: any) => {
            ...
            me.getRedisClient()
                ...
                .then((values: any) => {
                    Promise.all(
                        values.map((docId: any) =>
                            me
                                .doc(docId)
                                .get()
                                .then((data: any) => data && (data.id || data._id) ? data : Object.assign(data, {_id: docId})),
                        ),
                    )
                        .then((vals: any) => resolve(vals));
                })
                .catch(reject);
        });
    }

    scoringRange(min: any, max: any, data: any) {
        const me = this;
        const index = me.defaultRankingIndex;
        const key = me.getRankingPoolKey(index, data, this.pathName);
    ...
    rankFor(id: any) {
        const me = this;
        // /db/userStates:indexRank:rating.xp
        return new Promise((resolve: any, reject: (arg0: string) => void) => {
            // @ts-ignore
    ...
    pushJob(job: any) {
        const me = this;
        return new Promise((resolve: any, reject: ((arg0: any) => void) | undefined) => {
                ...
                me.getRedisClient()
                    ...
                    .then((nbAdd: number) => {
                        if (nbAdd === 1) {
                            ...
                            me.checkProcessedJobs(sId, (isPresent: any) => {
                                if (!isPresent) {
                                    me.getRedisClient()
                                        .lpush(`${me.pathName}:in`, item)
                                        .then(resolve)
    ...
    checkProcessedJobs(sId: any, cb: any) {
        const me = this;
    ...
    startWorker(userHandler: any, qRedisClient: any) {
        const me = this;
        process.on('exit', () => {
        ...
        function next() {
            ...
            function popped(err: Error, item: string) {
                ...
                function poppedFromProcessing(err: Error) {
    ...
    rangeByScore(scoreMin: any, scoreMax: any, limit: any) {
        const me = this;
    ...
    lrange(rankMin: any, rankMax: any) {
        const me = this;
        return new Promise((resolve: any, reject: any) => {
            // me.logger.log('calling get', me.pathName,me.id);
            if (me.id && me.docType === TYPE_SORTED_LIST) {
                ...
                me.getRedisClient()
                    ...
                    .then((all: any) => resolve(arrayKeyValueToHash(all)))
                    .catch(reject);
            } else {
                me.throw_error(`CANNOT set score on type ${me.docType}`, reject);
            }
        ...
        return new Promise((resolve: any, reject: any) => {
            ...
            if (me.id && me.docType === TYPE_LIST) {
                me.getRedisClient()
                    ...
                    .then((resp: number) => resolve(resp > 0))
                    .catch(reject);
            } else {
                me.throw_error(
    ...
    limit(l: (l: any) => RedisStore) {
        const c = this.clone();
        c.limit = l;
        return c;
    }

    orderBy(a: any, eq: any) {
        const c = this.clone();
        c.orderBy = [a, eq];
        return c;
    }
    ...
    async removeToBeDeletedKeys(data: any) {
        const me = this;
        const deletes: any = [];
        try {
            Object.keys(data)
            ...
            if (deletes.length) {
                const hKeys = await me.getRedisClient()
                    .hkeys(me.pathName);
                const delKeys: any = [];
                hKeys.forEach((key: string) => {
                    deletes.forEach((delk: string) => {
                ...
                if (delKeys.length) {
                    await Promise.all(
                        delKeys.map(async (delKey: string) => {
    ...
    async setDocData(data: string, opt: any = {merge: false, isReplicate: false, syncMainDb: false}) {
        try {
            if (opt.merge === false) {
            ...
            if (this.docType === TYPE_DOCUMENT) {
                ...
                if (this.indexes && this.indexes.length) {
                    this.indexes.forEach((keyPath: string) => {
            ...
            const checkErrorAndPath = (err: Error, path: string) => {
                if (err) {
    ...
    async delayedRemoveAll(timeout: number, ids: any[]) {
        await sleep(timeout);
        let items = ids;
    ...
    onSnapshot(options: any, callback: ((arg0: any) => void) | undefined) {
        const me = this;
        ...
        this.unsubscribe = nodeRedisPubPubsub.on(path, (data: any) => {
    ...
    setCollectionData(_data: any, opt: { merge?: boolean; isReplicate?: any; }): Promise<any> {
        const me = this;
        return new Promise((resolve: any, reject: any) => {
            let dataArray = _data;
            if (!is_array(_data)) {
    ...
    async set(_data: any, opt = {merge: true}) {
        //const me = this;
    ...
    async getMetadata(_data: any) {
        return this.getRedisClient()
            .hgetall(this.pathMetadata);
    }

    publishToAll(server: any) {
        const me = this;
