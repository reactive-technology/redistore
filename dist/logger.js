...
const getReqClientIp = (request) => {
    //return requestIp.getClientIp(request);
    const ip = request && request.info && request.info.remoteAddress;
    return ip ? `ip:${ip}` : '';
};
const toCommonLogFormat = (request, logType, caller) => {
    var application = request && request.auth && request.auth.credentials && request.auth.credentials.application || '', service = request && request.auth && request.auth.credentials && request.auth.credentials.service || '', deviceId = request && request.auth && request.auth.credentials && request.auth.credentials.deviceId || '', uuid = request && request.auth && request.auth.credentials && request.auth.credentials.deviceUUId || '', method = request && request.method && request.method.toUpperCase() || '', hrend = '';
    try {
    ...
    _log() {
        ...
        if (USE_WINSTON === 'yes' && wLogger[method]) {
            ...
            if (GOOGLE_APPLICATION_CREDENTIALS) {
                ...
                // A text log entry
                const entry = stackDriverLogger.entry({ labels, resource }, hostname + ' -> ' + args.map(arg => asString(arg)).join(' '));
