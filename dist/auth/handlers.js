    ...
    checkIn: (server) => {
        return (request, reply) => {
            ...
            try {
                ...
                token = JWT.sign(payload, PluginSettings.jwtPrivateKey, Object.assign(PluginSettings.jwtVerifyOptions, {
                    expiresIn
                }));
                const units = {
                    s: 1,
                ...
                // @ts-ignore
                (duration && unit && units[unit] && duration * units[unit] || 0))(expiresIn.replace(/\s*(\d+)\s*(.*)/g, '$1|$2')
                    .split('|'));
