...
//socket.on('message', function(){Logger.log('socketio ##################### burp')});
//socket.on('event', function(data){Logger.log('socketio event data',data)});
...
class RedisStoreClient {
  ...
  throw_error(msg, reject) {
    //Logger.error(`### ${msg} ##`, '==> proxy ', this.pathName, 'type', this.docType)
    try {
  ...
  doc(docName) {
    //Logger.log(' doc','on proxy ', this.pathName,'type',this.docType,'ref',this.ref.constructor.name);
  ...
  collection(collectionName, type = TYPE_COLLECTION) {
    //Logger.log(' collection','on proxy ', this.pathName,'type',this.docType);
  ...
  exists() {
    ...
    return new Promise(function (resolve, reject) {
        //Logger.log('calling get', me.pathName,me.id);
      }
    );
  }
