...
class WebSocketServer {
  ...
  async _start(routes, _subscriptions, hapiOptions) {
    ...
    const validation = (thepath) => {
      ...
      //console.log('params', thepath, params);
      return { params };
    };
    ...
    console.log('server http API');
    this.hapi.table()
      .forEach((route) => console.log(`${route.method}\t${route.path}`));
    console.log('server enabled subscription API ( server can publish on for clients to subscrible to)');
    subscriptions && subscriptions.forEach && subscriptions.forEach(subscription => console.log(subscription));
  }
}
