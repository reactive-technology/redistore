...
class WebSocketServer {
  ...
  async _start(routes, subscriptions) {
    const onDisconnection =  (a, b, c) => {
      this.hooks.onDisconnection && Promise.resolve(this.hooks.onDisconnection(a, b, c)).then();
    };
    const onConnection = async (a, b, c) => {
      this.hooks.onConnection && Promise.resolve(this.hooks.onConnection(a, b, c)).then();
    };
    const options = {
      onDisconnection,
      onConnection,
