...
class RedisStore {
  ...
  async getDocumentIndexes(pathIndexes = 'toto') {
    ...
    try {
      ...
      if (pathIndexes) {
        const obj = await this.getRedisClient().smembers(pathIndexes);
  ...
  expire(path) {
    ...
    this.getRedisClient().expire(path);
  }
  ...
  async updateRank(id, score) {
    ...
    try {
      ...
      if (this.id && this.parent.defaultRankingIndex) {
        ret = await this.getRedisClient().zadd(this.pathName, score, id);
        this.persist();
        return a;
      }
      this.throw_error(
              ...
              if (indexMap) {
                ...
                if (hasOneFieldOfCompositeIndex) {
                  if (merge && hasOneAbsentField) {
                    if (!fullData) {
                      fullData = await this.getRedisClient().hgetall(this.pathName);
                    }
  ...
  async getRankingCount0(data) {
    ...
    try {
      ...
      if (
        ...
        return await me.getRedisClient().zcount(key, '-inf','+inf');
      } else {
        console.trace('ERROR');
        err = `ranking field ${`${
          me.pathName
  ...
  async getRankingCount(data) {
    ...
    const count = await me.getRedisClient().zcount(key, '-inf','+inf');
  ...
  async getRank(data) {
    ...
    try {
      ...
      if (
        ...
        return await me.getRedisClient().zrank(key, me._id);
      } else {
        err = `ranking field ${`${
          me.parent.pathName
          }_${rankingField}`} not defined, not found in ${JSON.stringify(
  ...
  async removeToBeDeletedKeys(data) {
    ...
    try {
      ...
      if (deletes.length) {
        const hKeys = await me.getRedisClient().hkeys(me.pathName);
        const delKeys = [];
        hKeys.forEach(key => {
        ...
        if (delKeys.length) {
          await Promise.all(
            delKeys.map(async delKey => {
              const nbDeletes = await me.getRedisClient().hdel(me.pathName, delKey);
              if (nbDeletes) {
  ...
  async addParentIdIfNotExist() {
    try {
      const count = await this.parent.getRedisClient().sismember(
  ...
  async setDocData(data, opt = { merge: false }) {
    try {
      if (opt.merge === false) {
        const response = await this.getRedisClient().del(this.pathName);
        if (response === 1) {
      ...
      const ok = await this.getRedisClient().hmset(this.pathName, data);
      this.persist();
  ...
  async addItem(data) {
    ...
    try {
      const nb = await this.getRedisClient().sadd(this.pathCollection, data);
      return nb;
    } catch (e) {
  ...
  async removeItem(data) {
    // console.log('__sadd  path', me.pathName, 'data', data)
    try {
      const nb = await this.getRedisClient().srem(this.pathCollection, data);
      return nb;
    } catch (e) {
  ...
  async getItems() {
    // console.log('__sadd  path', me.pathName, 'data', data)
    try {
      const members = await this.getRedisClient().smembers(this.pathCollection);
      return members;
    } catch (e) {
      this.logger.error(e.message + ' _sadd err path ' + this.pathName);
      this.logger.error(e.stack);
  ...
  async setMetadata(_data) {
    ...
    try {
      ...
      if (data) {
        ...
        const res = await me.getRedisClient().hmset(me.pathMetadata, data);
  ...
  async getMetadata(_data) {
    return this.getRedisClient().hgetall(this.pathMetadata);
  }
}
