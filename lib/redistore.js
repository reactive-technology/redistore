...
class RedisStore {
  ...
  async getDocumentIndexes(pathIndexes = 'toto') {
    ...
    try {
      ...
      if (pathIndexes) {
        const obj = await this.redisClient.smembers(pathIndexes);
  ...
  async updateRank(id, score) {
    ...
    try {
      ...
      if (this.id && this.parent.defaultRankingIndex) {
        ret = await this.redisClient.zadd(this.pathName, score, id);
        this.persist();
        return a;
      }
      this.throw_error(
  ...
  formatIndexKey(index, data, path) {
    if (index.type) {
      this.logger.debug('index type undefined in store.js');
    }
    const key = `field(${index.type}:${
      index.rankingField
              ...
              if (indexMap) {
                ...
                if (hasOneFieldOfCompositeIndex) {
                  if (merge && hasOneAbsentField) {
                    if (!fullData) {
                      fullData = await this.redisClient.hgetall(this.pathName);
                    }
                    ...
                    .then(oldRankingKey => {
                      ...
                      if (oldRankingKey) {
                        if (rankingPoolKey !== oldRankingKey) {
                          // this.redisClient.del(oldRankingKey).then().catch(e => console.error(e));
                          this.redisClient
  ...
  async getRankingCount0(data) {
    ...
    try {
      ...
      if (
        ...
        return await me.redisClient.zcount(key, '-inf','+inf');
      } else {
        console.trace('ERROR');
        err = `ranking field ${`${
          me.pathName
  ...
  async getRank(data) {
    ...
    try {
      ...
      if (
        ...
        return await me.redisClient.zrank(key, me._id);
      } else {
        err = `ranking field ${`${
          me.parent.pathName
          }_${rankingField}`} not defined, not found in ${JSON.stringify(
        ...
        me.redisClient
          ...
          .then(nbAdd => {
            if (nbAdd === 1) {
              ...
              // me.redisClient.lpush(`${me.pathName}:in`, item, cb2promise(resolve, reject));
  ...
  async removeToBeDeletedKeys(data) {
    ...
    try {
      ...
      if (deletes.length) {
        const hKeys = await me.redisClient.hkeys(me.pathName);
        const delKeys = [];
        hKeys.forEach(key => {
        ...
        if (delKeys.length) {
          await Promise.all(
            delKeys.map(async delKey => {
              const nbDeletes = await me.redisClient.hdel(me.pathName, delKey);
              if (nbDeletes) {
  ...
  async addParentIdIfNotExist() {
    try {
      const count = await this.parent.redisClient.sismember(
  ...
  get(opt = { withContent: true }) {
    ...
    return new Promise((resolve, reject) => {
      const callback = obj => {
        const res = unflattenExt(obj, me.parent.types);
      ...
      if (me.id && me.docType === TYPE_DOCUMENT) {
        me.redisClient
          .hgetall(me.pathName)
          .then(callback)
          .catch(reject);
                    ...
                    return me
                      ...
                      .then(val => {
                        if (val._id) {
                          ...
                          membersWithValues.push({ _id: obj });
                        }
                      })
                      .catch(e => {
                        this.logger.error('Error : ' + e.message);
  ...
  async setDocData(data, opt = { merge: false }) {
    try {
      if (opt.merge === false) {
        const response = await this.redisClient.del(this.pathName);
        if (response === 1) {
      ...
      const ok = await this.redisClient.hmset(this.pathName, data);
      this.persist();
  ...
  async addItem(data) {
    ...
    try {
      const nb = await this.redisClient.sadd(this.pathCollection, data);
      return nb;
    } catch (e) {
  ...
  async removeItem(data) {
    // console.log('__sadd  path', me.pathName, 'data', data)
    try {
      const nb = await this.redisClient.srem(this.pathCollection, data);
      return nb;
    } catch (e) {
  ...
  async getItems() {
    // console.log('__sadd  path', me.pathName, 'data', data)
    try {
      const members = await this.redisClient.smembers(this.pathCollection);
      return members;
    } catch (e) {
      this.logger.error(e.message + ' _sadd err path ' + this.pathName);
      this.logger.error(e.stack);
  ...
  async setMetadata(_data) {
    ...
    try {
      ...
      if (data) {
        ...
        const res = await me.redisClient.hmset(me.pathMetadata, data);
  ...
  async getMetadata(_data) {
    return this.redisClient.hgetall(this.pathMetadata);
  }
}
