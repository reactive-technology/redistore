...
class RedisStore {
  ...
  async updateRank(id, score) {
    let ret = null;
    try {
      this.logger.log('calling get', me.pathName, me.id, this.parent.defaultRankingIndex);
                    ...
                    .then(oldRankingKey => {
                      ...
                      if (oldRankingKey) {
                        if (rankingPoolKey !== oldRankingKey) {
                          // this.getRedisClient().del(oldRankingKey).then().catch(e => this.logger.error(e));
                          this.getRedisClient()
  ...
  emit0() {
    setTimeout(this.emit2.bind(this), Math.random() * 100);
  }
        ...
        const pall = dataArray.map(
          elem =>
            me
              .doc(elem._id)
              .set(elem)
              .then() // .catch(e => this.logger.error(e, ++nbErr));
        );
        Promise.all(pall)
  ...
  async setMetadata(_data) {
    ...
    try {
      ...
      me.logger.debug('cannot set empty value', me.docName);
