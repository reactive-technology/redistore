  ...
  (async () => {
    try {
      ...
      const swaggerOptions = {
        info: {
          title: '94 seconds Rest API Documentation',
...
server.events.on('response', (request) => {
  ...
  if (request.path.indexOf('/health') === -1 && request.path.indexOf('/metrics') === -1) {
    ...
    console.info(commonInfo, `status=${status} [${hrend} ms] ${hrend > 1000 ? 'OPTIM!' : ''} ${hrend > 2000 ? 'SRV SLOW!' : ''} ${cached ? 'CACHED!' : ''}`);
  }
});
/*
server.ext('onPreResponse', (request) => {
  ...
  if (request.response.isBoom) {
    console.error('isBoom', JSON.stringify(request.response));//err.output.payload
  } else {
    request.response
