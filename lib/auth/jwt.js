  ...
  server.auth.strategy(Settings.app_webedia_auth_strategy, 'jwt', {
    ...
    validate: (decoded, request, callback) => {
      ...
      if (decoded.ip && ip != decoded.ip ) {
        console.error(request,'ip changed! expected ',ip,'found',decoded.ip,decoded);
        if(decoded.ip != '10.0.7.169')
        {
          return callback(null, false);
        }
        else{
          console.error(request,'exception detected : token redirect from our ip server are accepted');
        }
      }
      ...
      if (
        ...
        console.log(['jwt', 'auth', 'app'], 'device ', decoded.applicationKey, 'found!');
        try {
        ...
        console.log(['jwt', 'auth', 'app'], 'device ', decoded.applicationKey, 'found in users!');
      }

      console.log(['jwt', 'auth', 'app'], 'device', decoded.applicationKey, ' not found!');
      return callback(null, false);
    },
  });
  ...
  server.auth.strategy(Settings.admin_users_auth_strategy, 'jwt', {
    ...
    validate: (decoded, request, callback) => {
      ...
      if (userKeys.find(u => u.id === decoded.applicationKey)) {
        console.log(['jwt', 'auth', 'user'], 'device ', decoded.applicationKey, 'found!');
        try {
          return callback(null, true, decoded);
        } catch (e) {
          console.trace(e);
      ...
      console.log(['jwt', 'auth', 'user'], 'device', decoded.applicationKey, ' not found!');
      return callback(null, false);
    },
  });
